...
from db_utils import (get_all_classes, get_all_spells, get_class_by_name,
                      get_spells_by_class, get_race_for_ability, delete_spell_by_id,
                      get_all_races, add_new_race, add_new_spell, get_spell_by_name,
                      get_all_archetypes, get_archetype_by_name)
...
# route to fetch class by name (it's not a pokemon, only a madman would fetch a class by id...)
# changed route to take a string
@app.route("/classes/<string:name>", methods=["GET"])
def get_class_by_the_name(name):
    # passing argument name for the class you want to retrieve

    # return jsonify("TESTING ENDPOINT")
    return jsonify(get_class_by_name(name))
